...
export default function Capture() {
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        ...
        // Attach to both preview videos (MUTED ALWAYS to prevent desktop speaker echo)
  ...
  const takePhotos = async () => {
    ...
    // NOTE: these are taken sequentially; tiny differences can happen on fast motion.
  ...
  return (
    <div className="page">
      ...
      <div className="stageWrap">
        {/* IMPORTANT: stage is now true-centred (X + Y) so nothing gets cropped when viewport changes */}
        <div
          className="stage"
          style={{
            width: STAGE_W,
        ...
        >
          ...
          <div className="frame portraitFrame">
            <video
              ...
              className="previewVideo"
            />
            <div className="zoomPill zoomLeft">1.0×</div>
            <div className="tag tagRight">
              <span className="tagStrong">9:16</span>&nbsp; PORTRAIT
            </div>
          </div>

          {/* Logo (using your real SVG classes + only the logo group, so nothing “goes missing”) */}
          <div className="logo" aria-label="FlipCast Duo logo">
            <svg
          ...
          >
            ...
            {!micOn && <span className="micSlash" aria-hidden="true" />}
          </button>
      ...
      <style>{`
        ...
        /* Wrapper is just a positioning context now */
        .stageWrap{
          position:relative;
          width:100%;
          height:100%;
        ...
        /* TRUE centre (X+Y) so resizing never chops the top */
        .stage{
          position:absolute;
          left:50%;
          top:50%;
...
.logo{
  ...
  opacity: 0.95;
        ...
        .mic:not(.on){
          border-color: rgba(234,29,50,0.55);
          box-shadow: 0 0 0 2px rgba(234,29,50,0.16) inset;
        }
        .micIcon{
          font-size: 26px;
          opacity: 0.95;
